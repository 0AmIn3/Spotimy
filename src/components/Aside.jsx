      ...
      <section className="flex flex-col gap-[16px]">
        {
          playlist.map((item, idx) => (
            <Link to={`/playlist/${item.id}`} key={item.id} state={item.images[0].url} className="font-[500] font-Manrope text-[16px] text-[#B3B3B3] cursor-pointer duration-500 hover:text-[#fff6">
            {item.name}
          </Link>
          ))
        }
